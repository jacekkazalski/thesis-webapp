...
const {createRecipe, getRecipe, getRecipes, isAuthor, deleteRecipe, getRandomRecipe, updateRecipe} = require("../controller/recipeController");
const router = require("express").Router();
const upload = require('../config/multerConfig')

router.route('/create').post(authenticateToken, upload.single('image'), createRecipe)
router.route('/update').put(authenticateToken, upload.single('image'), updateRecipe)
