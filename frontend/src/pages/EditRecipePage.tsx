...
export default function EditRecipePage() {
  ...
  const handleQuantityChange = (ingredientId: number, quantity: string) => {
    setChosenIngredients((prev) =>
      prev.map((ing) =>
        ing.id_ingredient === ingredientId ? { ...ing, quantity: quantity } : ing,
      ),
    );
  };
  // Remove ingredient from chosen ingredients
  const handleRemoveIngredient = (ingredientId: number) => {
    const ingToRemove = chosenIngredients.find(
      (ing) => ing.id_ingredient == ingredientId,
    );
    if (ingToRemove) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      formData.append("id_recipe", recipeId || "");
      ...
      await axiosPrivate.put("/recipes/update", formData, {
  ...
  useEffect(() => {
    try {
      axiosPrivate.get("/ingredients").then((response) => {
        setAllIngredients(response.data.data);
      });
      // Fetch recipe and prefill form
      const fetchRecipe = async () => {
        const res = await axiosPrivate.get("/recipes/recipe", {
          params: { id_recipe: recipeId },
        });
        // response structure: { status, name, id_recipe, instructions, image_url, ingredients, author, rating }
        setName(res.data.name || "");
        setInstructions(res.data.instructions || "");
        setChosenIngredients(res.data.ingredients || []);
      };
      fetchRecipe();
    } catch (error) {
      console.log(error);
    }
  }, [axiosPrivate, recipeId]);
    ...
    <Box sx={{ width: "80%", p: 2, alignContent: "center", mx: "auto" }}>
      ...
      >
        <Paper elevation={2} sx={{ flex: 2, p: 2 }}>
          <Stack spacing={2}>
            <Typography variant={"h4"}>Edytuj przepis</Typography>
            {errorMsg && (
              <Typography color={"warning.dark"}>{errorMsg}</Typography>
            )}
            <TextField
              required
            ...
            <Grid container spacing={1} alignItems="center" sx={{ mb: 1 }}>
              ...
              <Grid size={5}>
                <TextField
                  ...
                  onChange={(e) =>
                    handleQuantityChange(ingredient.id_ingredient, e.target.value)
                  }
                />
              </Grid>
            </Grid>
